...
f = open('results/volkov_mm_batched_%s.tsv' % args.devicename, 'r')
...
for timeinfo in times:
    ...
    batchsize = int(name.split('_')[1].replace('b', ''))
...
plt.savefig('/tmp/volkov_mm_batched_%s.png' % deviceNameSimple, dpi=150)
