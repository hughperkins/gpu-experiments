...
experiments = [
    {'name': 'blocksize{blocksize}', 'code': code_template}
]
...
for experiment in experiments:
    ...
    grid_x = 1
    while blockSize <= 512:
        name = experiment['name'].format(blocksize=blockSize)
        if args.printptx:
            clearComputeCache()
...
f = open('/tmp/globalwrite_blocksize_%s.tsv' % di.deviceSimpleName, 'w')
