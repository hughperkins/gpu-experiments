...
f = open('results/globalwrite_blocksize_%s.tsv' % args.devicename, 'r')
...
for timeinfo in times:
    name = timeinfo['name']
    family = 'a'
    blockSize = int(name.replace('blocksize', ''))
...
plt.savefig('/tmp/globalwrite_blocksize_%s.png' % deviceNameSimple, dpi=150)
